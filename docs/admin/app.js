...
const PUBLIC_VIEW_URL = `${SUPABASE_URL}/rest/v1/paws_public_tracker?select=*&order=intake_date.desc&limit=5`;

// In a real prod environment, the PIN would be entered on a login screen 
// and stored in a secure session or variable.
let SAVED_PIN = "2499"; // Default for demo
    ...
    if (data && data.length > 0) {
      const listHtml = data.map(dog => `
        <div class="recent-item">
          <span>${dog.name} (<code>${dog.paws_ref}</code>)</span>
